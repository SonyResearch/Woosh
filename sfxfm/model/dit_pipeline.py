"""
Defines full DiT pipeline with preprocessing, modality blocks
and postprocessing.
"""
...
class InputProcessing(nn.Module):
    def __init__(self, args: DiTArgs):
        ...
        # Replace the masked tokens with a learnable parameter
    ...
    def pad_description(
        ...
        """
        Pads the description with a learnable parameter.
        """
...
class DiTPipeline(torch.nn.Module):
    def __init__(
        ...
        """
        Wraps preprocessing, DiT layers and postprocessing together.
        """
        super().__init__()
    ...
    def forward(
        self, x: torch.Tensor, t, cond: Dict[str, torch.Tensor], mask: torch.Tensor
    ) -> DictTensor:
...
class SFXFlow(DiTPipeline):
    """
    Same as Flux, but uses MMMBlocks only. Adds singlestream blocks.
