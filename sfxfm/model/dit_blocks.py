...
class CrossAttention(nn.Module):
    """
    Same cross attention as the one in ditv2
    only the keys of the different elements are provided at init time
    ...
    def __init__(
        ...
        kv_key: str = "x",
        mod_key: Optional[str] = None,
        freqs_cis_q_key: Optional[str] = None,
        freqs_cis_kv_key: Optional[str] = None,
        mask_key: Optional[str] = None,
    ):
        super().__init__()
        self.dim = args.dim
        ...
        self.use_modulation = mod_key is not None
        self.use_rotary = freqs_cis_q_key is not None and freqs_cis_kv_key is not None
        if self.use_modulation:
        ...
        self.kv_key = kv_key
        self.mod_key = mod_key
        self.freqs_cis_q_key = freqs_cis_q_key
        self.freqs_cis_kv_key = freqs_cis_kv_key
        self.mask_key = mask_key
    ...
    def forward(
        ...
        freqs_cis_q = d[self.freqs_cis_q_key] if self.freqs_cis_q_key else None
        freqs_cis_kv = d[self.freqs_cis_kv_key] if self.freqs_cis_kv_key else None
        mask = d[self.mask_key] if self.mask_key else None

        bsz, seqlen_q, _ = x.size()
        bsz, seqlen_kv, _ = y.size()
...
class MultiModalCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        apply_rope_1: bool = False,
        apply_rope_2: bool = False,
    ):
        super().__init__()

        self.modality_1 = modality_1
        self.modality_2 = modality_2
        self.apply_rope_1 = apply_rope_1
        self.apply_rope_2 = apply_rope_2
    ...
    def forward(
        ...
        x_1 = d[self.modality_1.x_key]
        x_2 = d[self.modality_2.x_key]

        res_1 = x_1
        res_2 = x_2

        q_1, k_1, v_1, q_rope_1, k_rope_1, attn_mask_1, gate_1 = (
            self.modality_1.precompute(d, apply_rope=self.apply_rope_1)
        )
        q_2, k_2, v_2, q_rope_2, k_rope_2, attn_mask_2, gate_2 = (
            self.modality_2.precompute(d, apply_rope=self.apply_rope_2)
        )
        if self.apply_rope_1:
            assert q_rope_1 is not None and k_rope_1 is not None
            q_1, k_1 = q_rope_1, k_rope_1
        if self.apply_rope_2:
            assert q_rope_2 is not None and k_rope_2 is not None
            q_2, k_2 = q_rope_2, k_rope_2
        ...
        attn_mask = torch.cat([attn_mask_1, attn_mask_2], dim=3)
        ...
        # get x and y back
        x_1, x_2 = z[:, :, :seqlen_1], z[:, :, seqlen_1:]
        assert x_2.size(2) == seqlen_2

        x_1 = self.modality_1.out_proj(rearrange(x_1, "b h s d -> b s (h d)"))
        x_2 = self.modality_2.out_proj(rearrange(x_2, "b h s d -> b s (h d)"))

        if self.modality_1.use_modulation:
            x_1 = x_1 * gate_1
        if self.modality_2.use_modulation:
            x_2 = x_2 * gate_2

        x_1 = x_1 + res_1
        x_2 = x_2 + res_2
        d[self.modality_1.x_key] = x_1
        d[self.modality_2.x_key] = x_2
        ...
        if store_attentions:
            ...
            seqlen = attn_mask.sum((1, 2, 3)).max().item()
            # crop q and k to the right size
            q = q[:, :, :seqlen, :]
            k = k[:, :, :seqlen, :]
...
class MMBlock(nn.Module):
    ...
    BE CAREFUL, this is not the same as the one in ditv2 which contains NO TEXT MLP
    ...
    def __init__(
        ...
        self.attn = MultiModalCrossAttention(
            ...
            apply_rope_1=apply_rope_1,
            apply_rope_2=apply_rope_2,
        )
        self.ffn_x = MLP(args, main_key=modality_1.x_key, mod_key=modality_1.mod_key)
        self.ffn_description = MLP(
            args, main_key=modality_2.x_key, mod_key=modality_2.mod_key
        )
    ...
    def forward(
        ...
        d = self.ffn_x.forward(d)
        d = self.ffn_description.forward(d)
        return d


class UMBlock(nn.Module):
    """
    Transformer block combining attention and feed-forward layers for only ONE modality.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.attn = modality
        self.ffn = MLP(args, main_key=modality.x_key, mod_key=modality.mod_key)
