...
def check_if_file_exists_else_download(path, fname2link=FILE2URL, chunk_size=1024):
    ...
    if not path.exists():
        ...
        if link is None:
            raise ValueError(
                f"Cant find the checkpoint file: {path}.",
...
class MotionFormer(VisionTransformer):
    ...
    def __init__(
        ...
        if self.ckpt_path is not None:
            ...
            if self.ckpt_path.endswith(tuple(mformer_ckpt2cfg.keys())):
                ...
                if s1_cfg is not None:
                    s1_vfeat_extractor_ckpt_path = (
                        s1_cfg.model.params.vfeat_extractor.params.ckpt_path
                    )
        ...
        if (self.ckpt_path is not None) and (not was_pt_on_avclip):
            ...
            if (
                len(_ckpt_load_status.missing_keys) > 0
                or len(_ckpt_load_status.unexpected_keys) > 0
            ):
                logging.warning(
        ...
        if self.extract_features:
            ...
            if self.factorize_space_time:
                if agg_space_module == "TransformerEncoderLayer":
                    ...
                    raise ValueError(
                        f"agg_space_module {agg_space_module} is not supported."
                    )
                if agg_time_module == "TransformerEncoderLayer":
                    ...
                    raise ValueError(
                        f"agg_time_module {agg_time_module} is not supported."
                    )
            ...
            if add_global_repr:
                if agg_segments_module == "TransformerEncoderLayer":
                    ...
                    pos_max_len = (
                        max_segments if max_segments is not None else 16
                    )  # 16 = 10sec//0.64sec + 1
                    ...
                    raise ValueError(
                        f"agg_segments_module {agg_segments_module} is not supported."
                    )
        ...
        if was_pt_on_avclip:
            ...
            if (
                len(_load_status.missing_keys) > 0
                or len(_load_status.unexpected_keys) > 0
            ):
                logging.warning(
                    f"Loading exact vfeat_extractor ckpt from {self.ckpt_path} failed. \n"
...
class BaseEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor = None):
        ...
        if x_mask is not None:
            ...
            assert x_mask_w_cls.dtype == x_mask_w_cls.bool().dtype, (
...
class Synchformer(nn.Module):
    # source: https://huggingface.co/spaces/Heartsync/sfx/blob/1354a37ca82704c0ecb1a0c98ef9a88f9ebff45c/mmaudio/ext/synchformer/synchformer.py
    def __init__(self):
        super().__init__()
...
def encode_video_with_sync(
    synchformer, x: torch.Tensor, batch_size: int = -1, pad=True
) -> torch.Tensor:
    assert synchformer is not None, "Synchformer is not loaded"
    # x: (B, T, C, H, W) H/W: 384
    # partition the video
    segment_size = 16
    step_size = 8
    ...
    if pad:
        # pad the video to make it divisible by segment_size
        t = x.shape[1]
        pad_size = segment_size
        x = torch.nn.functional.pad(x, (0, 0, 0, 0, 0, 0, pad_size, 0, 0, 0))
        # logging.info(f"Padded video to {x.shape[1]} frames, {x.shape}")
    ...
    x = x[:, :original_T]  # remove padding if any
...
if __name__ == "__main__":
    ...
    features = model(vid).detach().cpu()

    frames = torch.rand([4, 253, 3, 224, 224]).cuda()  # Batch, T, C, H, W

    a = encode_video_with_sync(model, frames, 4)
