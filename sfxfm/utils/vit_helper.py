...
def qkv_attn(q, k, v, tok_mask: torch.Tensor = None):
    ...
    if tok_mask is not None:
        ...
        sim = sim.masked_fill(
            tok_mask.view(BSH, 1, N) == 0, float("-inf")
        )  # 1 - broadcasts across N
...
class DividedAttention(nn.Module):
    ...
    def forward(
        self, x, einops_from, einops_to, tok_mask: torch.Tensor = None, **einops_dims
    ):
        ...
        if tok_mask is not None:
            ...
            tok_mask = (
                tok_mask.unsqueeze(1).expand(-1, h, -1).reshape(-1, tok_mask.shape[1])
            )
        ...
        if tok_mask is not None:
            ...
            mask_ = rearrange(
                mask_, f"{einops_from} -> {einops_to}".replace(" d", ""), **einops_dims
            )
            cls_mask = repeat(
                cls_mask, "b () -> (b r) ()", r=r
            )  # expand cls_mask across time or space
    ...
    def __init__(self, n_input, n_classes, n_hidden=512, p=0.1):
        ...
        if n_hidden is None:
            ...
            self.block_forward = nn.Sequential(
                nn.Dropout(p=p), nn.Linear(n_input, n_classes, bias=True)
            )
        else:
