...
from sfxfm.module.model.transformer import (
    ...
    register_backward_nan_check_hook,
)
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        seq_embed_dim = condition_processor.seq_embed_dim
        ...
        if seq_embed_dim > 0:
            # Sequence embedding
            self.to_seq_embed = nn.Sequential(
                nn.Linear(seq_embed_dim, embed_dim, bias=False),
                nn.SiLU(),
                nn.Linear(embed_dim, embed_dim, bias=False),
            )
    ...
    def _forward(
        ...
        **kwargs,
        ...
        if seq_embed is not None:
            # Project the sequence embedding to the embedding dimension
            seq_embed = self.to_seq_embed(seq_embed)
        ...
        if input_concat_cond is not None:
            ...
            register_backward_nan_check_hook(x, "dit_input_concat_cond")
        ...
        register_backward_nan_check_hook(x, "dit_preprocess_conv")

        x = rearrange(x, "b c t -> b t c")

        extra_args = {}

        extra_args["sequence_cond"] = seq_embed
        ...
        if (
            self.transformer_type == "x-transformers"
            or self.transformer_type == "continuous_transformer"
        ):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass for the DiffusionTransformer model.

        Args:
            x (torch.Tensor): Input tensor.
            t (torch.Tensor): Timestep tensor.
            cond (Dict[str, torch.Tensor]): Dictionary containing conditioning tensors. The following keys are expected: cross_attn_cond, cross_attn_cond_mask, global_embed
            negative_cross_attn_cond (torch.Tensor, optional): Negative cross-attention conditioning tensor.
            negative_cross_attn_mask (torch.Tensor, optional): Mask for negative cross-attention conditioning.
            input_concat_cond (torch.Tensor, optional): Input concatenation conditioning tensor.
            negative_global_embed (torch.Tensor, optional): Negative global embedding tensor.
            prepend_cond (torch.Tensor, optional): Prepend conditioning tensor.
            prepend_cond_mask (torch.Tensor, optional): Mask for prepend conditioning.
            cfg_scale (float, optional): Scale for classifier-free guidance. Default is 1.0.
            cfg_dropout_prob (float, optional): Dropout probability for classifier-free guidance. Default is 0.0.
            causal (bool, optional): Whether to use causal mode. Default is False.
            scale_phi (float, optional): Scaling factor for phi. Default is 0.0.
        ...
        """
        register_backward_nan_check_hook(x, "dit_x")
        # extract conditions
