...
def dpp_hash(mystring):
    return int(hashlib.sha256(mystring.encode("utf-8")).hexdigest()[:16], 16) - (
        1 << 63
    )


def get_collate_fn(
    columns=["audio_length", "id", "hash_caption"],
    precompute_spec=False,
    data_filling="pad",
    **kwargs,
):
    def collate_fn(batch):
        ...
        for b in batch:
            if not isinstance(b, dict):
                log.warning(f"Collate function received non-dict entry: {b}")
                print(f"\nCollate function received non-dict entry: {b}\n")
        # filter out non-dict entries from batch and log warning if found
        batch = [b for b in batch if isinstance(b, dict)]
        if len(batch) < orig_batch_len:
            log.warning(
                f"Collate function removed {orig_batch_len - len(batch)} non-dict entries from batch"
            )
            # if distributed, add duplicate random entries to batch
            if is_distributed():
                n_missing = orig_batch_len - len(batch)
                for _ in range(n_missing):
                    batch.append(batch[np.random.randint(len(batch))])
                log.warning(
                    f"Added {n_missing} random entries to batch to ensure equal batch sizes during distributed training"
                )

        result = [{k: b[k] for k in columns} for b in batch]
        result = torch.utils.data.default_collate(result)
        # make a batch of chunks as batch['audio']
        lengths = torch.tensor([e["audio"].size(-1) for e in batch])

        audio = [b["audio"] for b in batch]
        if data_filling == "repeatpad":
            # repeatpad
            max_len = lengths.max()
            n_repeat = (max_len / lengths).int()
            audio = [b["audio"].repeat(r) for r, b in zip(n_repeat, batch)]
            lengths = torch.tensor([a.size(-1) for a in audio])
            audio = torch.nn.utils.rnn.pad_sequence(audio, batch_first=True)
        elif data_filling == "repeat":
            max_len = lengths.max()
            n_repeat = (max_len / lengths).int()
            audio = [
                b["audio"].repeat(r + 1)[:max_len] for r, b in zip(n_repeat, batch)
            ]
            lengths = torch.tensor([a.size(-1) for a in audio])
        ...
        result["audio"] = audio
        result["audio_length_sample"] = lengths

        if precompute_spec:
            spec_data = [
                kwargs["spectogram_fn"](sample={}, audio_data=b["audio"]) for b in batch
            ]
            spec_data = torch.utils.data.default_collate(spec_data)
            result = {**result, **spec_data}

        # result["captions"] = [
        #     s["captions"]
        #     if isinstance(s["captions"], (tuple, list))
        #     else [s["captions"]]
        #     for s in batch
        # ]  # captions as list of lists
        result["captions"] = [s["captions"] for s in batch]  # captions as list of lists

        # indices of original batch for each caption; used for repeating audio features in case of multiple captions per audio
        result["indices"] = [i for i, b in enumerate(result["captions"]) for _ in b]
        # flatten list of lists for captions
        result["captions"] = [
            caption for sublist in result["captions"] for caption in sublist
        ]
...
def lr_warmup_decay_lambda(
    ...
    """
    This function calculates the learning rate based on the epoch/step number.

    Parameters:
    epoch (int): The current epoch number.
    min_lr_factor (float): The minimum factor for the learning rate.
    warmup_length (int): The length of the warmup phase in epochs.
    rampdown_stop (int): The epoch at which the rampdown phase stops.
    rampdown_start (int, optional): The start epoch of the rampdown phase.
                                        Defaults to None, meaning it starts from the beginning of training.
    warmup_type (str, optional): The type of warmup. "linear" or "cosine".
                                    Defaults to "linear".
    rampdown_type (str, optional): The type of rampdown. "cosine".
                                    Defaults to "cosine".

    Returns:
    float: The calculated learning rate multiple.
    """

    if rampdown_start is None:
        rampdown_start = warmup_length

    if epoch < warmup_length:
        if warmup_type == "linear":
            return epoch * 1.0 / warmup_length
